...
// Taken from: https://github.com/CorrM/CleanCheat/blob/main/src/CleanCheat/MemoryManager.h
...
class MemoryManager {
  ...
  public:
    std::vector<void *> PatternScan(void *moduleBaseAddress, const std::string &signature,
                                    const uint32_t maxResultCount = 1) {
        std::vector<void *> ret;
        const auto dosHeader = static_cast<PIMAGE_DOS_HEADER>(moduleBaseAddress);
        const auto ntHeaders =
            reinterpret_cast<PIMAGE_NT_HEADERS>(reinterpret_cast<uintptr_t>(moduleBaseAddress) + dosHeader->e_lfanew);
